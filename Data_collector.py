...
# Builds a high level data structure for storing user details by implementing a list of Dictionaries
...
# Outputs data stored in our Database
...
count = 1
answer = "no"
while answer == "no":                      # While there is another user
    ...
    response =input("Use suggested password? [y/n] > ").lower()
    ...
    if response == "y":
        ...
        while len(password) <7:
            ...
            display_details(first_name,last_name,email)
            database_build(first_name,last_name,email,password)                         #Append data to database
            last_user =input("Are you the last user [y/n] > ").lower()
            if last_user == "y":
                answer= "yes"
            else:
